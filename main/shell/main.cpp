...
{
	CASPAR_LOG(info) << L"############################################################################";
	CASPAR_LOG(info) << L"Famous Server is distributed by the Zqvideo";
	...
	CASPAR_LOG(info) << L"http://www.zqvideo.com/";
	CASPAR_LOG(info) << L"############################################################################";
	CASPAR_LOG(info) << L"Starting Famous Video and Graphics Playout Server " << env::version();
...
int main(int argc, char** argv)
{
	...
	std::wstring config_file_name(L"FamousServer.config");
	...
	try 
	{
		...
		CASPAR_LOG(info) << "Successfully shutdown Famous Server.";
	...
	{
		CASPAR_LOG_CURRENT_EXCEPTION();
		CASPAR_LOG(fatal) << L"Unhandled exception in main thread. Please report this error on the Zqvideo.";
		boost::this_thread::sleep_for(boost::chrono::milliseconds(1000));
		std::wcout << L"\n\nFamous Server will automatically shutdown. See the log file located at the configured log-file folder for more information.\n\n";
