...
#include <protocol/vdcp/VDCPProtocolStrategy.h>
#include <protocol/vdcp/vdcp_command_repository.h>
#include <protocol/vdcp/VDCPCommandsImpl.h>

#include <protocol/cii/CIIProtocolStrategy.h>
#include <protocol/clk/CLKProtocolStrategy.h>
#include <protocol/util/AsyncEventServer.h>
#include <protocol/util/AsyncEventServer_Serial.h>
...
struct server::impl : boost::noncopyable
{
	...
	std::shared_ptr<vdcp::vdcp_command_repository>      vdcp_command_repo_;
	std::vector<spl::shared_ptr<IO::AsyncEventServer>>	async_servers_;
	std::vector<spl::shared_ptr<IO::AsyncEventServer_Serial>>	async_servers_serial;
	...
	~impl()
	{
		...
		vdcp_command_repo_.reset();
		primary_amcp_server_.reset();
		async_servers_.clear();
		async_servers_serial.clear();
	...
	// afd-table
	int get_afd_code_by_string(const std::wstring& afd_string)
	{
		// SD
		if (afd_string == L"sd_16_9_top")return 2;
		if (afd_string == L"sd_14_9_top")return 3;
		if (afd_string == L"sd_greater_than_16_9_center")return 4;
		if (afd_string == L"sd_full_frame")return 8;
		if (afd_string == L"sd_16_9_center")return 10;
		if (afd_string == L"sd_14_9_center")return 11;
		if (afd_string == L"sd_4_3_with_14_9_alternative_center")return 13;
		if (afd_string == L"sd_16_9_with_14_9_alternative_center")return 14;
		if (afd_string == L"sd_16_9_with_4_3_alternative_center")return 15;

		// HD
		if (afd_string == L"hd_greater_than_16_9_center")return 4;
		if (afd_string == L"hd_full_frame")return 8;
		if (afd_string == L"hd_4_3_center")return 9;
		if (afd_string == L"hd_16_9_with_16_9_image_protected")return 10;
		if (afd_string == L"hd_14_9_center")return 11;
		if (afd_string == L"hd_4_3_with_14_9_alternative_center")return 13;
		if (afd_string == L"hd_16_9_with_14_9_alternative_center")return 14;
		if (afd_string == L"hd_16_9_with_4_3_alternative_center")return 15;
	...
	void setup_channels(const boost::property_tree::wptree& pt)
	{
		...
		for (auto& xml_channel : pt | witerate_children(L"configuration.channels") | welement_context_iteration)
		{
			...
			format_desc.delayFrames = delayFrames;
			//wxg20170104logoKiller
			...
			{
				afd = L"invalid-afd";
				CASPAR_LOG(error) << L"Warning,afd-mode value is invalid, eg:<afd-mode>stretch</afd-mode>";
			}
			format_desc.afd_mode = afd;
			//---------------------------------------------
			...
			if (afd == L"table")
			{
				auto afd_table_default = xml_channel.second.get(L"default", L"stretch");

				// afd-table
				if (afd_table_default != L"stretch" && afd_table_default != L"cut" && afd_table_default != L"add")
				{
					afd_table_default = L"stretch";
					CASPAR_LOG(info) << L"Warning: The default value of afd-table is invalid, famous server will do the default processing, please check the configuration file!";
				}
				format_desc.table_afd_default = afd_table_default;
			}
			...
			if (afd == L"table")
			{
				BOOST_FOREACH(auto& afd_msg, xml_channel.second.get_child(L"table"))
				{
					...
					//判断transform是否有效，如果出现无效的配置，报错并且将afd的值设置成stretch
					if (!(afd == L"auto" || afd == L"add" || afd == L"cut" || afd == L"stretch"))
					{
						transform = L"stretch";
						CASPAR_LOG(error) << L"Warning, <table>node error,check<transform>node";
					}
					channel->add_afd_item(
						spl::make_shared<video_channel::afd_item>(
							afd_code, transform
							));
				}
			}
	...
	{
		if (true/*!pt.get(L"configuration.thumbnails.generate-thumbnails", true)*/)
			return;
	...
	void setup_controllers(const boost::property_tree::wptree& pt)
	{
		...
		vdcp_command_repo_ = spl::make_shared<vdcp::vdcp_command_repository>(
			channels_,
			thumbnail_generator_,
			media_info_repo_,
			system_info_provider_repo_,
		...
		vdcp::register_commands(*vdcp_command_repo_);
		...
		for (auto& xml_controller : pt | witerate_children(L"configuration.controllers") | welement_context_iteration)
		{
			...
			else if(boost::iequals(name, L"serial"))
			{
	...
	{
		using namespace IO;
		std::wstring protocol_expresion = boost::to_upper_copy(name);
		...
		else if (protocol_expresion.find(L"VDCP") != std::wstring::npos)
		{
			...
			{
				CASPAR_LOG(error) << L"vdcp protocol: xml.config miss parameters about channel - layer";
			}
		}
...
spl::shared_ptr<protocol::vdcp::vdcp_command_repository> server::get_vdcp_command_repository() const { return spl::make_shared_ptr(impl_->vdcp_command_repo_); }
