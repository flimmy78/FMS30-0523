...
class image_renderer
{
	...
	tbb::concurrent_bounded_queue<std::shared_ptr<uint8_t>>												temp_buffers_;
	core::video_format_desc																				format_desc_;
	tbb::cache_aligned_allocator<uint8_t>                                                               cc_memory_pool;
	tbb::cache_aligned_allocator<uint8_t>                                                               blend_memory_pool;
...
public:	
	std::future<array<const std::uint8_t>> operator()(std::vector<item> items, const core::video_format_desc& format_desc)
	{
		...
		uint8_t* pdest = (uint8_t*)cc_memory_pool.allocate(format_desc.size);
		memset(pdest, 0, format_desc.size);
		auto result = std::shared_ptr<uint8_t>(pdest, [&](uint8_t* p) {cc_memory_pool.deallocate(p, format_desc.size); });

		if(format_desc.field_mode != core::field_mode::progressive)
		{			
			draw(items, pdest, format_desc.width, format_desc.height, core::field_mode::upper);
			draw(items, pdest, format_desc.width, format_desc.height, core::field_mode::lower);
		}
		else
		{
			draw(items, pdest, format_desc.width, format_desc.height,  core::field_mode::progressive);
		}

		temp_buffers_.clear();
		return make_ready_future(array<const std::uint8_t>(pdest, format_desc.size, true, result));
	}
...
private:
	...
	void convert(std::vector<item>& source_items, int width, int height)
	{
		...
		{			
			...
			uint8_t* pdest = (uint8_t*)cc_memory_pool.allocate(width*height * 4);
			temp_buffers_.push(std::shared_ptr<uint8_t>(pdest, [&](uint8_t* p) {cc_memory_pool.deallocate(p, width*height * 4); }));
			{
				auto dest_av_frame = ffmpeg::create_frame();

//2.0			avpicture_fill(reinterpret_cast<AVPicture*>(dest_av_frame.get()), pdest, PIX_FMT_BGRA, width, height);
//ffmpeg3.0
				avpicture_fill(reinterpret_cast<AVPicture*>(dest_av_frame.get()), pdest, AV_PIX_FMT_BGRA, width, height);
...
public:
	...
	core::mutable_frame create_frame(const void* tag, const core::pixel_format_desc& desc, const core::audio_channel_layout& channel_layout)
	{
		...
		{
			uint8_t* pdest = (uint8_t*)cc_memory_pool.allocate(plane.size);
			auto buf = std::shared_ptr<uint8_t>(pdest, [&](uint8_t* p) {cc_memory_pool.deallocate(p, plane.size); });
			buffers.push_back(array<std::uint8_t>(buf.get(), plane.size, true, buf));
		}
