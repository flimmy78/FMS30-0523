...
public:
	...
	//cue first frame
	//----------------
	std::future<void> load_ext(int index, const spl::shared_ptr<frame_producer>& producer, bool preview, bool can_show_first_frame, const boost::optional<int32_t>& auto_play_delta)
	{
		return executor_.begin_invoke([=]
		{
			layer& curlayer = get_layer(index);
			if (can_show_first_frame && (curlayer.is_paused() || curlayer.is_arrive_eom()))
			{
				curlayer.load(producer, true, auto_play_delta);
			}
			else
			{
				curlayer.load(producer, preview, auto_play_delta);
			}
		}, task_priority::high_priority);
	...
	int get_layerstatus(int index)
	{
		return get_layer(index).getstatus();
	}

	void set_logokiller(int index, std::wstring paramstr)
	{
		executor_.begin_invoke([=]
		{
			layer& curlayer = get_layer(index);
...
std::future<void> stage::load_ext(int index, const spl::shared_ptr<frame_producer>& producer, bool preview, bool can_show_first_frame, const boost::optional<int32_t>& auto_play_delta) { return impl_->load_ext(index, producer, preview, can_show_first_frame, auto_play_delta); }//cue first frame
...
int				  stage::get_layerstatus(int index) { return impl_->get_layerstatus(index); }
