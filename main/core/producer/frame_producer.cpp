...
int frame_producer_base::producer_type()
{
	return impl_->producer_type();
}
...
spl::shared_ptr<core::frame_producer> frame_producer_registry::create_producer(const frame_producer_dependencies& dependencies, const std::vector<std::wstring>& params) const
{
	...
	//add by zibj 20170104 若未找到params[0]同名素材，则判断params[0]是否为文件夹，查找文件夹下的同名素材。
	auto params_copy = params;
	if (producer == frame_producer::empty())
	{
		if (!params.empty() && !boost::contains(params.at(0), L"://")
			&& !boost::contains(params.at(0), L"/") && !boost::contains(params.at(0), L"\\"))
		{
			boost::filesystem::path dir_path(env::media_folder() + L"/" + params.at(0));
			if (boost::filesystem::is_directory(dir_path))
			{
				params_copy[0] = params.at(0) + L"/" + params.at(0);
				producer = do_create_producer(dependencies, params_copy, producer_factories);
				if (producer == frame_producer::empty())
				{
					params_copy = params;//若未找到params[0]子文件夹下的同名素材，则还原参数。
				}				
			}			
		}
	}
	//add by zibj 20170104 end
	...
	if (!params.empty() && !boost::contains(params.at(0), L"://"))
	{
		...
		{
			//auto params_copy = params; //comment out by zibj 20170104 
