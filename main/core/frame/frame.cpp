...
{			
	...
	AFD_COMMAND                 out_afd_command_;//afd command
	int							src_afd_msg;// Active format descriptor code from *.mxf or sdi.
	impl(const void* tag)
	...
	impl(
		...
		, geometry_(frame_geometry::get_default())
		, should_record_age_(false)
		, out_afd_command_(afdcmd)//afd command
		, src_afd_msg(src_afd_msg)/*src afd msg.*/
	{
		if (desc.format != core::pixel_format::bgra)
			CASPAR_THROW_EXCEPTION(not_implemented());
...
const_frame::const_frame(
	...
	const core::pixel_format_desc& desc,
	const core::audio_channel_layout& channel_layout,
	AFD_COMMAND afd_command,
	int src_afd_msg)
	: impl_(new impl(std::move(image), std::move(audio_data), tag, desc, channel_layout,afd_command, src_afd_msg)) {}
...
int const_frame::src_afd_msg() const
{
	if (impl_->size()!=0)
	{
		if (impl_->src_afd_msg >= 0)
		{
			return impl_->src_afd_msg;
		}
	}
...
AFD_COMMAND* const_frame::afd_command() const
{
	if (impl_->size()!=0)
	{
		if (impl_->out_afd_command_.afd_aspect_ratio_ >= 0 && impl_->out_afd_command_.afd_command_ >= 0)
		{
			return &(impl_->out_afd_command_);
		}
	}
	return nullptr;
}
