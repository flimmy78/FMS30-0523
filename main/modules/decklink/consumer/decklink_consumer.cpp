...
public:
		...
		, vanc_output_(config.vanc_output_)//config vanc
	{
		...
		if (vanc_output_)
		{
			enable_video(get_display_mode(output_, format_desc_.format, bmdFormat10BitYUV, bmdVideoOutputVANC, will_attempt_dma_));
		}
		else
		{
		...
		for (int n = 0; n < buffer_size_; ++n)
		{
			...
			vanc_output_ ? schedule_next_video_vanc(core::const_frame::empty()) : schedule_next_video(core::const_frame::empty());
	...
	void enable_video(BMDDisplayMode display_mode)
	{
		...
		if (vanc_output_)
		{
			if (FAILED(output_->EnableVideoOutput(display_mode, bmdVideoOutputVANC)))
				BOOST_THROW_EXCEPTION(caspar_exception() << msg_info(print() + L" Could not enable fill video output."));
			V210Encoder_ = std::shared_ptr<V210Encoder>(new V210Encoder(format_desc_, config_.key_only));
		}
		else
		{
			if (FAILED(output_->EnableVideoOutput(display_mode, bmdVideoOutputFlagDefault)))
	...
	void schedule_next_audio(const T& audio_data)
	{
		...
		/*CASPAR_LOG(info) << " audio_data.size() = " << audio_data.size();
		CASPAR_LOG(info) << " out_channel_layout_.num_channels = " << out_channel_layout_.num_channels;
		CASPAR_LOG(info) << "sample_frame_count = " << sample_frame_count;*/
		audio_container_.push_back(std::vector<int32_t>(audio_data.begin(), audio_data.end()));
		//CASPAR_LOG(info) << "audio_scheduled_ = " << audio_scheduled_;
	...
	//modify vanc
	//---------------------------------------------
	void schedule_next_video_vanc(core::const_frame frame)
	{
		...
		if (frame.afd_command())
		{
			...
			int nCmd = frame.afd_command()->afd_command_;
			std::shared_ptr<AFDData> pAfd(new AFDData());
			pAfd->SetAspectRatioOfCodedFrame((AFDData::AspectRatio)nAR);
			pAfd->SetActiveFormatDescription((AFDData::AFD)nCmd);
			//std::vector<unsigned char> myAfdData = pAfd->GetHDYCBytesData(AncillaryData::ADM_20Bit_Y);
			std::vector<unsigned char> myAfdData = pAfd->GetHDYCBytesData((format_desc_.name == L"PAL" || format_desc_.name == L"pal") ? AncillaryData::ADM_10Bit : AncillaryData::ADM_20Bit_Y);
			IDeckLinkVideoFrameAncillary* pAncData;
			output_->CreateAncillaryData(bmdFormat10BitYUV, &pAncData);
			void* buf;
			pAncData->GetBufferForVerticalBlankingLine(13, &buf);
