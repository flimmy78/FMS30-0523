...
//logo_killer20161229
struct  externalParas
{
	int afd_aspect_ratio;
	int afd_command;
...
//	tbb::concurrent_bounded_queue<std::pair<core::draw_frame, uint32_t>>	frame_buffer_;
	int64_t												frame_number_				= 0;
	uint32_t											file_frame_number_			= 0;
	
	boost::thread										thread_;
	tbb::atomic<bool>                                   is_runing_;
	int													video_index_;
	int64_t												pts_start_;
	unsigned											source_afd_code;// Active format descriptor code from *.mxf
	int                                                 afd_aspect_ratio_;//afd command
	int                                                 afd_command_;//afd command
	int											        seekFrameCount_;
	bool                                                bsyncFrame_;
	boost::mutex                                        buffer_mutex_;
	bool                                                bHasSeeked_;
	bool                                                bfirstSyncFrame_;
public:
	explicit ffmpeg_producer(
			...
			const externalParas extreanlPas)
		...
		
	{
		...
		bsyncFrame_ = false;
		bHasSeeked_ = false;
		bfirstSyncFrame_ = false;
		source_afd_code = input_.source_afd_code;
		...
		try
		{
			...
			//logo killer20161229
			//-----------------------
			if (!extreanlPas.logo_killer_enable&&format_desc.wstr_logo_killer != L"")
			{

				set_logokiller(format_desc.wstr_logo_killer);
			}
			else {
				video_decoder_->set_logo_killer_params(extreanlPas.logo_killer_enable, extreanlPas.logo_killer_left, extreanlPas.logo_killer_top,
					extreanlPas.logo_killer_width, extreanlPas.logo_killer_height, extreanlPas.logo_killer_is_smooth, extreanlPas.logo_killer_smooth_value);
			}
		...
		muxer_.reset(new frame_muxer(framerate_, std::move(audio_input_pads), frame_factory, format_desc, channel_layout, source_afd_code,filter, true));

		video_index_ = av_find_best_stream(input_.context().get(), AVMEDIA_TYPE_VIDEO, -1, -1, 0, 0);
		auto fps = read_fps(*input_.context().get(), 0.0);
		auto pStream = input_.context()->streams[video_index_];
		pts_start_ = 0;
		if (!is_url())
		{
		...
		for (unsigned stream_index = 0; stream_index < input_.context()->nb_streams; ++stream_index)
		{
			...
			if (i < audio_decoders_.size())
				audio_decoders_[i]->set_FirstVideoTimecode(pts_start_audioUints + videoTimeStampmills_toAudioUints);//音频总能seek到
			i++;
		}		
		// wxg20161019增加解码线程
		is_runing_ = true;
	...
	void run()
	{
		...
		CASPAR_LOG(info) << L" quit Run decode thread ! ";
	}
	...
	void try_decode_frame()
	{
		...
		if (video && !bHasSeeked_)
		{
			auto pStream = input_.context()->streams[video_index_];
			int64_t vpkt_pts = 0;
			if (video->pkt_pts != AV_NOPTS_VALUE)
				vpkt_pts = video->pkt_pts - pStream->start_time;
			if (vpkt_pts < pts_start_ )
			{
				auto fps = read_fps(*input_.context().get(), 0.0);
				...
				if (bsyncFrame_)
					seekFrameCount_--;
				boost::this_thread::sleep_for(boost::chrono::milliseconds(10));
				if (seekFrameCount_ > -1 /*&& bsyncFrame_*/)
				{
					if (audio.size() >= 1 && audio[0]->capacity() >= 1)
					{
						muxer_->push(audio);
					}
					return;
	...
	void set_logokiller(std::wstring  paramstr)
	{
		...
		if (paramstr != L"")
		{
			...
			if (logo_killer_str != "")
			{
				...
				logokillers.logo_killer_enable = (bool)atoi(strEnable.c_str());
				if (logokillers.logo_killer_enable)
				{
					find = logo_killer_str.find_first_of(",");
					std::string strLeft = logo_killer_str.substr(0, find);
					logo_killer_str = logo_killer_str.erase(0, find + 1);
					logokillers.logo_killer_left = atoi(strLeft.c_str());

					find = logo_killer_str.find_first_of(",");
					std::string strTop = logo_killer_str.substr(0, find);
					logo_killer_str = logo_killer_str.erase(0, find + 1);
					logokillers.logo_killer_top = atoi(strTop.c_str());

					find = logo_killer_str.find_first_of(",");
					std::string strWidth = logo_killer_str.substr(0, find);
					logo_killer_str = logo_killer_str.erase(0, find + 1);
					logokillers.logo_killer_width = atoi(strWidth.c_str());

					find = logo_killer_str.find_first_of(",");
					std::string strHeight = logo_killer_str.substr(0, find);
					logo_killer_str = logo_killer_str.erase(0, find + 1);
					logokillers.logo_killer_height = atoi(strHeight.c_str());

					find = logo_killer_str.find_first_of(",");
					std::string strIsSmooth = logo_killer_str.substr(0, find);
					logo_killer_str = logo_killer_str.erase(0, find + 1);
					logokillers.logo_killer_is_smooth = (bool)atoi(strIsSmooth.c_str());

					std::string strSmoothValue = logo_killer_str;
					logokillers.logo_killer_smooth_value = atoi(strSmoothValue.c_str());
				}
				video_decoder_->set_logo_killer_params(logokillers.logo_killer_enable, logokillers.logo_killer_left, logokillers.logo_killer_top,
					logokillers.logo_killer_width, logokillers.logo_killer_height, logokillers.logo_killer_is_smooth, logokillers.logo_killer_smooth_value);
...
{
	std::vector<std::wstring> params = Params;
	...
	externalParas  logo_Kill_Afd_paras;
	if (logo_killer_str != "")
	{
		logo_killer_str = logo_killer_str.erase(0, 1);
		logo_killer_str = logo_killer_str.erase(logo_killer_str.length() - 1, 1);
		try
		{
			...
			std::string strEnable = logo_killer_str.substr(0, find);
			logo_killer_str = logo_killer_str.erase(0, find + 1);
			logo_Kill_Afd_paras.logo_killer_enable = (bool)atoi(strEnable.c_str());
			if (logo_Kill_Afd_paras.logo_killer_enable)
			{
				find = logo_killer_str.find_first_of(",");
				std::string strLeft = logo_killer_str.substr(0, find);
				logo_killer_str = logo_killer_str.erase(0, find + 1);
				logo_Kill_Afd_paras.logo_killer_left = atoi(strLeft.c_str());

				find = logo_killer_str.find_first_of(",");
				std::string strTop = logo_killer_str.substr(0, find);
				logo_killer_str = logo_killer_str.erase(0, find + 1);
				logo_Kill_Afd_paras.logo_killer_top = atoi(strTop.c_str());

				find = logo_killer_str.find_first_of(",");
				std::string strWidth = logo_killer_str.substr(0, find);
				logo_killer_str = logo_killer_str.erase(0, find + 1);
				logo_Kill_Afd_paras.logo_killer_width = atoi(strWidth.c_str());

				find = logo_killer_str.find_first_of(",");
				std::string strHeight = logo_killer_str.substr(0, find);
				logo_killer_str = logo_killer_str.erase(0, find + 1);
				logo_Kill_Afd_paras.logo_killer_height = atoi(strHeight.c_str());

				find = logo_killer_str.find_first_of(",");
				std::string strIsSmooth = logo_killer_str.substr(0, find);
				logo_killer_str = logo_killer_str.erase(0, find + 1);
				logo_Kill_Afd_paras.logo_killer_is_smooth = (bool)atoi(strIsSmooth.c_str());

				std::string strSmoothValue = logo_killer_str;
				logo_Kill_Afd_paras.logo_killer_smooth_value = atoi(strSmoothValue.c_str());
			}
		}
		catch (...)
		{
	...
	std::wstring afd;
	int afd_aspect_ratio = -1;
	int afd_command = -1;
	for (std::vector<std::wstring>::iterator it  = params.begin();it !=params.end();)
	{
		std::vector<std::wstring>::iterator itnext = it;
		itnext++;
		if (boost::iequals(L"AFD",*it))
		{
			if (itnext!=params.end())
			{
				afd = *itnext;
			}
			std::string afd_str = u8(afd);
			std::string::size_type found = afd_str.find("-");
			if (found >= 1 && found < 100)
			{
				std::string strA = afd_str.substr(0, found);
				std::string strC = afd_str.substr(found + 1, afd.size() - found - 1);
				afd_aspect_ratio = atoi(strA.c_str());
				afd_command = atoi(strC.c_str());
				params.erase(itnext);
			}
			params.erase(it);
			break;
	...
	logo_Kill_Afd_paras.afd_aspect_ratio = afd_aspect_ratio;
	logo_Kill_Afd_paras.afd_command = afd_command;
