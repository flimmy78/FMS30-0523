...
//3.0
	...
	bool find_key_packet(spl::shared_ptr<AVPacket> &packet1)
	{
		...
		int count = nframe + 1;
		int nCount = 0;
		while (count <= static_cast<int>(in_))
		{
	...
	//	return ret == AVERROR_EOF || ret == AVERROR(EIO) || frame_number_ >= length_; // av_read_frame doesn't always correctly return AVERROR_EOF;
		if (out_ == std::numeric_limits<uint32_t>::max())
		{
			return ret == AVERROR_EOF || ret == AVERROR(EIO) || file_frame_number_ >= out_;
		}
		else
		{
...
uint32_t input::start_position() { return impl_->in_; }//Omn Seek
