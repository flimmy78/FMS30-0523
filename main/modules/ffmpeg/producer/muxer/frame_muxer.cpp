...
struct frame_muxer::impl : boost::noncopyable
{
	...
	impl(
		...
		, m_source_afd_code(source_afd_mode)
	...
	void push(const std::shared_ptr<AVFrame>& video_frame, int sdi_afd = -1)
	{
		if (!video_frame)
			return;
		...
		else
		{
			...
			if (filter_)
			{
				...
				for (auto& av_frame : filter_->poll_all())
				{
					...
					int src_afd = -1;
					if (m_source_afd_code != -1)src_afd = m_source_afd_code;
					if (sdi_afd != -1)src_afd = sdi_afd;

					//--------------------------------
					if ( !boost::iequals(boost::to_lower_copy(format_desc_.afd_mode),L"invalid-afd"))
					{
						if (format_desc_.afd_mode == L"auto")
						{
							video_streams_.back().push(make_frame_auto_afd(this, av_frame, *frame_factory_, audio_channel_layout_,src_afd,format_desc_));
						}
						else
						{
							video_streams_.back().push(make_frame_afd(this, av_frame, *frame_factory_, audio_channel_layout_, src_afd, format_desc_, m_p_video_channel));
						}
					}
					else
					{
		...
		if (video_streams_.back().size() > max_stream_size)
			CASPAR_LOG(error) << L"video-stream overflow. This can be caused by incorrect frame-rate. Check clip meta-data.";
	...
	void push(const std::vector<std::shared_ptr<core::mutable_audio_buffer>>& audio_samples_per_stream)
	{
		...
		if (audio_streams_.back().size() > max_stream_size * audio_cadence_.front() * audio_channel_layout_.num_channels)
			CASPAR_LOG(error) << L"audio-stream overflow. This can be caused by incorrect frame-rate. Check clip meta-data.";
