...
std::wstring loadbg_command(command_context& ctx)
{
	...
	if (mediainfo && mediainfo->clip_type==L"MOVIE")
	{
	...
	//cue first frame
	//------------------
	bool can_show_first_frame = true;
	if(contains_param(L"SHOW_FF_OFF",ctx.parameters))
	{
		can_show_first_frame = false;
		remove_param(L"SHOW_FF_OFF", ctx.parameters); 
	}
	//------------------
...
// 		channel->stage().load(ctx.layer_index(), pFP2, false); // TODO: LOOP
	...
	//cue first frame
	//------------------
	if (can_show_first_frame)
	{
		if (auto_play)
			channel->stage().load_ext(ctx.layer_index(), pFP2, false, can_show_first_frame, transitionInfo.duration); // TODO: LOOP
		else
			channel->stage().load_ext(ctx.layer_index(), pFP2, false, can_show_first_frame); // TODO: LOOP
	}
	else
	{
		if (auto_play)
...
void add_describer(core::help_sink& sink, const core::help_repository& repo)
{
	...
	sink.para()
		...
		->see(L"the Famous config file")->text(L".");
	sink.para()
...
std::wstring set_command(command_context& ctx)
{
	...
	else if(name == L"LOGO_KILLER")
	{
		if (ctx.parameters.size() != 2 )
		{
			return L"403 SET error \r\n";
		}
		else
		{
			try
			{
				...
				int i = 1;
				bool findout = false;
				for (BOOST_AUTO(it, child.begin()); it != child.end();i++,++it)
				{
					if (channelindex == i)
					{
						auto label = it->first;
						auto &sd = it->second;
						auto logo = sd.get<std::wstring>(L"logo-killer", L"0,-1,-1,-1,-1,0,1");
						std::wstring logo_killer_str;
						logo_killer_str = L"{" + logo + L"}";
						if (logo_killer_str != value)
						{
							logo_killer_str = value;
							logo_killer_str = logo_killer_str.erase(0, 1);
							logo_killer_str = logo_killer_str.erase(logo_killer_str.length() - 1, 1);
							sd.put<std::wstring>(L"logo-killer", logo_killer_str);
							findout = true;
							break;
						}
					}
				...
				if (findout)
				{
					pt.put_child(L"configuration.channels", child);
					boost::property_tree::xml_parser::xml_writer_settings<std::wstring> settings(' ', 3, (L"utf-8"));
					boost::property_tree::xml_parser::write_xml("FamousServer.config", pt, std::locale(), settings);
				}
			}
			catch (...)
			{
				CASPAR_THROW_EXCEPTION(user_error() << msg_info(L"Invalid logo_killer"));
			}
			return L"202 SET LOGO_KILLER OK\r\n";
		}
	}
...
void mixer_straight_alpha_describer(core::help_sink& sink, const core::help_repository& repo)
{
	...
	sink.para()->code(L"FamousServer.config")->text(L" needs to be configured to enable the feature.");
	sink.para()->text(L"Examples:");
...
void channel_grid_describer(core::help_sink& sink, const core::help_repository& repo)
{
	...
	sink.para()
		...
		->code(L"FamousServer.config")->text(L" for this to work correctly.");
}
...
void bye_describer(core::help_sink& sink, const core::help_repository& repo)
{
	...
	sink.para()
		...
		->text(L"on the machine Famous is running on then this will achieve the same as the ")->see(L"KILL")->text(L" command.");
}
...
void restart_describer(core::help_sink& sink, const core::help_repository& repo)
{
	...
	sink.para()
		...
		->text(L"Intended for use in combination with ")->code(L"famous_auto_restart.bat");
}
...
void layerstatus_describer(core::help_sink& sink, const core::help_repository& repo)
{
	sink.short_description(L"layer status ,use index .");
	sink.syntax(LR"(layerstatus [video_channel:int]{-[layer:int]|-0} )");
	sink.para()
		->text(L"Layer is the property of the channel.The 10 layer is dedicated to broadcast the layer number  ")
		->text(L"Through the layer of the query, you can know the current status of the server and broadcast the state");
	sink.para()->text(L"Examples:");
	sink.example(L">> LAYERSTATUS 1-10");
	sink.para()->text(L"Note: See ")->see(L"LAYERSTATUS")->text(L" for additional details.");
}

std::wstring layerstatus_command(command_context& ctx)
{
	std::wstring replyString;
	replyString.append(L"202 LAYERSTATUS OK");
	int ret_value = ctx.channel.channel->stage().get_layerstatus(ctx.layer_index());
	replyString.append(L"\r\n");


	replyString.append(L"LAYERSTATUS ");
	replyString.append(boost::lexical_cast<std::wstring>(ctx.channel_index+1));
	replyString.append( L"-");
	replyString.append( boost::lexical_cast<std::wstring>(ctx.layer_index()));
	replyString.append(L" ");
	replyString.append(boost::lexical_cast<std::wstring>(ret_value));
	replyString.append(L"\r\n");
	return replyString;
}
