#ifndef MV_264_INDEX_GENERATOR
#define MV_264_INDEX_GENERATOR
...
{
public:
   C264Index(uint64_t in_ui64Position, uint64_t in_ui64DisplayPosition, uint64_t in_ui64CodedPosition, uint64_t in_ui64Size, uint32_t in_ui32Type, std::string in_strType)
   ...
   {
      std::stringstream oLineString;
      oLineString << m_ui64Position
         << "," << m_ui64DisplayPosition
         << "," << m_ui64CodedPosition
         << "," << m_ui64Size
         << "," << m_ui32Type
         << "," << m_strType;

      return oLineString.str();
   }
...
#else
   bool DumpToFile(std::string in_wstrIndexFilename, uint32_t in_ui32Width, uint32_t in_ui32Height, std::string in_strFrameRate, std::string in_strScanMode, std::string in_strAspectRatio, uint32_t in_ui32BitCount)
#endif
   {
      try
      {
